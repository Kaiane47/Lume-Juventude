...
eventTabs.forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    eventBoxes.forEach(box => {
      box.style.display = box.getAttribute('data-event') === event ? 'block' : 'none';
    });
  });
});

// Cronômetro da Vigília
function updateVigiliaTimer() {
  const vigiliaDate = new Date("October 3, 2025 20:00:00").getTime();
  const now = new Date().getTime();
  const distance = vigiliaDate - now;

  if (distance <= 0) {
    document.getElementById("vigilia-timer").innerHTML = "🔥 Começou!";
    clearInterval(timer);
    return;
  }
...
updateVigiliaTimer();
const timer = setInterval(updateVigiliaTimer, 1000);